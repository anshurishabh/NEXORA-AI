...
function historyBlock(convo) {
  ...
  const lines = prior.map((m) => (m.role === 'user' ? 'User' : 'NEXORA') + ': ' + (m.content || '').slice(0, 800));
  return (
    '\n\n[FULL CONVERSATION HISTORY — everything said earlier in this same chat. Always use this to remember ' +
    'names, facts, topics, or context the user mentioned before, for as long as this chat continues.]\n' + lines.join('\n')
  );
}
...
async function planAndDelegate(effectiveQuery, hasDocument, usageTotals) {
  const planPrompt =
    ...
    'specific subtask instruction for it. If conversation history is included, use it to understand what the ' +
    'user is actually asking about now — a short follow-up like "what about X" refers back to that history. ' +
...
function buildSynthesisPrompt(originalGoal, successful) {
  ...
  return (
    ...
    'user\u2019s original goal: "' + originalGoal.slice(0, 500) + '". Combine their contributions below into one clear, ' +
...
async function runOrchestration(query, options) {
  ...
  const effectiveQuery = query + (opts.memoryContext || '') + (opts.historyContext || '');
...
router.post('/stream', async (req, res) => {
  ...
  try {
    ...
    const planned = await planAndDelegate(trimmedQuery + memCtx + histCtx, false, usageTotals);
