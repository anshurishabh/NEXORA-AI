...
async function planAndDelegate(effectiveQuery, hasDocument, usageTotals) {
  const planPrompt =
    ...
    (hasDocument
      ? ' A document was attached, so include the "document" agent.'
      : "");
  ...
  try {
    ...
    console.error(
      "[orchestrate] PLAN JSON PARSE FAILED. Raw output was:",
      planResult.text,
    );
    throw new Error(
      "Groq responded, but not in the expected JSON format. Raw: " +
        planResult.text.slice(0, 200),
    );
  }

  let agents = (plan.agents || [])
    .filter((a) => SELECTABLE_AGENTS.includes(a))
    .slice(0, 4);
  if (agents.length === 0)
    agents = hasDocument ? ["document", "content"] : ["research", "content"];
    ...
    agents.map(async (agentKey) => {
      ...
      if (needsSearch) {
        try {
          ...
          searchResultsUsed = results.map((r) => ({
            title: r.title,
            link: r.link,
          }));
          if (results.length) {
          ...
          console.error(
            '[orchestrate] web search failed for "' + subtask + '":',
            e.message,
          );
        }
      }
      ...
      const agentSystemPrompt =
        ...
        " agent for NEXORA AI. " +
        info.desc +
        " Complete this subtask as part of a larger collaborative answer. Match your depth and length to what the " +
        ' request actually needs — be thorough and detailed for complex, open-ended, or "explain/describe" style asks, ' +
        " and shorter for simple factual ones. Use examples, structure, or bullet points where they genuinely help. " +
        " Do not artificially pad or artificially compress." +
        todayLine +
        formatLine +
        " Do not output JSON.";
      ...
      try {
        ...
        console.error(
          '[orchestrate] AGENT "' + agentKey + '" FAILED:',
          err.message,
        );
        return {
          agent: agentKey,
...
function buildSynthesisPrompt(originalGoal, successful) {
  const synthesisInput = successful
    .map(
      (t) => "[" + AGENT_INFO[t.agent].label.toUpperCase() + "]\n" + t.output,
    )
    .join("\n\n");
  return (
    "You are the Verifier agent for NEXORA AI. Specialist agents have each contributed a piece toward the " +
    'user\u2019s original goal: "' +
    originalGoal.slice(0, 300) +
    '". Combine their contributions below into one clear, ' +
    "well-organized, non-redundant final answer. Write with the depth and length the user\u2019s question deserves — " +
    'thorough and richly detailed for complex or "explain/describe/how does X work" style questions, concise for ' +
    "simple factual ones. Do not compress away genuinely useful detail just to hit a word count. Use short paragraphs, " +
    "headers, or bullet points where they improve readability. You may use **bold** sparingly. " +
...
async function runOrchestration(query, options) {
  ...
  const { agents, agentTraces } = await planAndDelegate(
    effectiveQuery,
    !!opts.hasDocument,
    usageTotals,
  );
  ...
  if (successful.length === 0) {
    const firstReason =
      (agentTraces.find((t) => !t.ok) || {}).error || "unknown error";
    throw new Error(
      "All specialist agents failed to respond. First error: " + firstReason,
    );
  }

  const synthesisPrompt = buildSynthesisPrompt(effectiveQuery, successful);
  const synthResult = await runProvider(
    PROVIDER,
    LEVEL,
  ...
  return {
    agents,
    agentTraces,
    responseText: synthResult.text.trim(),
    usage: usageTotals,
  };
}
...
router.post("/", async (req, res) => {
  ...
  const trimmedQuery =
    (query || "").trim() ||
    (hasImage ? "Describe this image and anything useful about it." : "");
  const db = readDB();
  const memCtx = memoryBlock(db);

  let convo = conversationId
    ? db.conversations.find((c) => c.id === conversationId)
    : null;
  if (!convo) {
    convo = {
      id: makeId(),
  ...
  try {
      ...
      agentTraces = [
        {
          ...
          output: responseText,
          tokens: result.usage.totalTokens,
          subtask: trimmedQuery,
          sources: [],
        },
      ];
    } else {
      const docQuery = hasDocument
        ? 'The user attached a document named "' +
          attachment.name +
          '". Document content:\n' +
          (attachment.text
          ...
          "\n\nUser request: " +
          (trimmedQuery ||
            "Summarize this document and highlight anything important.")
        : trimmedQuery;

      const result = await runOrchestration(docQuery, {
    ...
    responseText =
      errorCode === "NO_KEY"
        ...
        : "I couldn't complete this task right now (" +
          err.message +
...
router.post("/stream", async (req, res) => {
  ...
  try {
    ...
    const planned = await planAndDelegate(
      trimmedQuery + memCtx,
      false,
      usageTotals,
    );
